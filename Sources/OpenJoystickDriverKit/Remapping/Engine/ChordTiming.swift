...
extension RemappingDeviceState {
  ...
  func selectedChords(releasing source: RemappingSource? = nil) -> [RemappingChord] {
    ...
    return candidates.enumerated().compactMap { index, chord in
      ...
      guard
        isClosing
          || !candidates.prefix(index).contains(where: {
  ...
  func matches(_ chord: RemappingChord) -> Bool {
    ...
    let activeChordSources = effectiveChords.filter { activeChords.contains($0.id) }.reduce(
      into: Set<RemappingSource>()
    ) { $0.formUnion($1.sources) }
    let consumedByOtherRecognizers = consumedChordSources.subtracting(activeChordSources)
    guard
      activeChords.contains(chord.id) || chord.sources.isDisjoint(with: consumedByOtherRecognizers)
    else { return false }
