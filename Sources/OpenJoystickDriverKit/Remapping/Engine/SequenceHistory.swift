...
extension RemappingDeviceState {
  var effectiveSequences: [RemappingSequence] {
    profile.sequences
      + activeLayers.flatMap { id in profile.layers.first { $0.id == id }?.sequences ?? [] }
  }

  var sequenceHistoryDeadline: UInt64? {
    guard !sequenceHistory.contains(where: \.awaitingChord), let first = sequenceHistory.first,
  ...
  mutating func pruneSequenceHistory(at uptime: UInt64) {
    ...
    let capacity =
      (sequences.map { $0.sources.count }.max() ?? 0)
    ...
    let retentionUptime = min(
      uptime,
      sequenceHistory.first(where: \.awaitingChord)?.uptime ?? uptime
    )
