...
extension GIPParserTests {
  ...
  func testParseStackedFramesFromOneTransfer() throws {
    ...
    let events = try parser.parse(
      data: ProtocolPacketFixtures.GIP.inputPacket(payload: Data(repeating: 0, count: 14))
  ...
  func testParseChunkFrameHeaderWithoutBlockingFollowingFrame() throws {
    ...
    #expect(
      try parser.parse(data: ProtocolPacketFixtures.GIP.inputPacket(payload: pressed)).contains(
        .buttonPressed(.a)
      )
    )
  }
  ...
  func testParseMainInputShareButton() throws {
    ...
    payload[14] = 0
    let releaseEvents = try parser.parse(
      data: ProtocolPacketFixtures.GIP.inputPacket(payload: payload, sequence: 1)
    )
