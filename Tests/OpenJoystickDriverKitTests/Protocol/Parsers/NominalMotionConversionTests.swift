...
struct NominalMotionConversionTests {
  ...
  func nintendoConvertsEverySampleIntoTheStableHardwareFrame(
    ...
    for sample in samples {
      ...
      let expected =
        layout == .rightJoyCon ? [140.01, -210.02, -70.01] : [-140.01, 210.02, -70.01]
  ...
  @Test
  func dualShockFourUsesSonyNominalUnitsWithoutChangingRawSamples() throws {
    ...
    #expect(
      sample.physicalReading?.gyroscopeDegreesPerSecond == ControllerMotionVector(x: 1, y: -1, z: 0)
    )
