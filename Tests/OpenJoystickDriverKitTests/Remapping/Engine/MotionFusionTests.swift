...
struct MotionFusionTests {
  ...
  @Test(arguments: [
    ControllerMotionVector(x: 0, y: 1, z: 0), ControllerMotionVector(x: 1, y: 0, z: 0),
  ...
  @Test
  func yawIntegrationUsesDegreesAndKeepsUnitOrientation() throws {
    var fusion = RemappingMotionFusion()
    _ = fusion.update(gyro: zero, acceleration: up, deltaTime: 0)
    for _ in 0..<100 {
      _ = fusion.update(
        gyro: ControllerMotionVector(x: 0, y: 90, z: 0),
        acceleration: up,
        deltaTime: 0.01
      )
    }
  ...
  func pitchTraceSeparatesGravityFromLinearAcceleration() throws {
    ...
    let pulse = fusion.update(
      gyro: zero,
      acceleration: ControllerMotionVector(x: 2, y: 0, z: 0),
      deltaTime: 0.01
    )
