...
struct RemappingPendingChordTests {
  @Test
  func completedChordConsumesBothConstituentActions() {
  ...
  @Test
  func unmatchedPassthroughTapProducesPressThenNeutral() {
    var state = RemappingEngineState()
  ...
  @Test
  func completedSmallerChordWaitsForLargerCandidate() {
    var state = RemappingEngineState()
  ...
  @Test
  func largerCandidateExpiryCommitsCompletedSmallerChord() {
    var state = RemappingEngineState()
    let profile = overlappingProfile()
    _ = state.process(
      events: [.buttonPressed(.a), .buttonPressed(.b)],
  ...
  @Test
  func releasingCompletedSmallerChordCommitsItBeforeRelease() {
    var state = RemappingEngineState()
    let profile = overlappingProfile()
    _ = state.process(
      events: [.buttonPressed(.a), .buttonPressed(.b)],
  ...
  @Test
  func largerChordReleaseDoesNotActivateConsumedSmallerChord() {
    var state = RemappingEngineState()
    let profile = overlappingProfile()
    _ = state.process(
  ...
  @Test
  func unmatchedDirectionReleaseReplaysItsAxisSample() {
    var state = RemappingEngineState()
  ...
  @Test
  func modifierChordConsumesBindingsAndSequenceHistory() {
    ...
    let profile = RemappingProfile(
      ...
      bindings: [
        RemappingBinding(
          ...
          additionalActions: [
            RemappingAction(destination: .keyboard(key: .x, modifiers: []), behavior: .toggle)
          ]
        ), RemappingBinding(source: .button(.east), destination: .keyboard(key: .b, modifiers: [])),
      ],
      chords: [
        RemappingChord(sources: Set(sources), destination: .keyboard(key: .c, modifiers: []))
      ],
      sequences: [
        RemappingSequence(
          sources: sources,
  ...
  @Test
  func shorterHigherPriorityWindowDeterminesCommitDeadline() {
    var state = RemappingEngineState()
  ...
  private func overlappingProfile(
    smallerWindow: Double = 50,
    largerWindow: Double = 100
  ) -> RemappingProfile {
    let base = profile()
