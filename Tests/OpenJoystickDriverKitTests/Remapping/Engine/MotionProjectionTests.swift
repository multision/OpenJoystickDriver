...
struct MotionProjectionTests {
  @Test
  func flatPosePreservesPitchAndYawInEverySpace() throws {
  ...
  @Test
  func sidewaysPoseDistinguishesAllThreeSpaces() throws {
  ...
  @Test
  func tiltedPlayerYawRelaxesWithoutExceedingCombinedRate() throws {
    let gravity = ControllerMotionVector(x: 0, y: -1, z: -1)
    let gyro = ControllerMotionVector(x: 0, y: 10, z: 0)
    let player = try #require(
      RemappingMotionProjection.project(gyro, gravity: gravity, space: .player)
    )
    let world = try #require(
      RemappingMotionProjection.project(gyro, gravity: gravity, space: .world)
    )
  ...
  @Test
  func degenerateGravityAndInvalidTuningAreRejected() {
    ...
    #expect(
      RemappingMotionProjection.project(gyro, gravity: zero, space: .local, yawRelaxation: .nan)
        == nil
    )
    #expect(
      RemappingMotionProjection.project(
