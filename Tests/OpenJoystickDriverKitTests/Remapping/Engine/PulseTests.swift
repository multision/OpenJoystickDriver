...
struct RemappingPulseTests {
  @Test
  func pulseEndsAtItsDeadlineAfterPhysicalRelease() throws {
  ...
  @Test
  func newPressExtendsPulseAndDrainCancelsItsDeadline() {
    var state = RemappingEngineState()
    let profile = profile()
    let identifier = device()
    _ = state.process(
    ...
    #expect(
      state.nextScheduledTick(after: 100_000_000, continuousIntervalNanoseconds: 1) == 150_000_000
    )
  ...
  func pulseDeadlineSaturatesWithoutOverflow() {
    ...
    #expect(
      state.nextScheduledTick(after: UInt64.max - 10, continuousIntervalNanoseconds: 1)
        == UInt64.max
    )
