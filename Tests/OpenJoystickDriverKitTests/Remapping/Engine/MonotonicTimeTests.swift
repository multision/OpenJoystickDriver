...
struct RemappingMonotonicTimeTests {
  @Test
  func backwardTickDoesNotReverseTurboPhase() {
    ...
    #expect(
      state.nextScheduledTick(after: 10_000_000, continuousIntervalNanoseconds: 1) == 100_000_000
    )
    #expect(
      state.process(
  ...
  @Test
  func backwardInputCannotShortenNewPulse() {
    var state = RemappingEngineState()
  ...
  @Test
  func controllerClocksRemainIndependent() {
    var state = RemappingEngineState()
  ...
  @Test
  func backwardReleaseStillReleasesHeldOutput() {
    var state = RemappingEngineState()
    let profile = profile(
      binding: RemappingBinding(
        source: .button(.south),
