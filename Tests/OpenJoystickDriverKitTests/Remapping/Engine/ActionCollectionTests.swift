...
struct RemappingActionCollectionTests {
  @Test
  func oneControlDrivesMixedActionsWithIndependentReleaseBehavior() throws {
    let profile = profile(
  ...
  @Test
  func independentHoldDeadlinesDoNotReplaceEachOther() {
    let profile = profile(
      binding: RemappingBinding(
        source: .button(.south),
  ...
  @Test
  func continuousAdditionalActionUsesTheSameInputSample() {
    let profile = profile(
      binding: RemappingBinding(
