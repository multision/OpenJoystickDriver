...
from Scripts.Platform.capabilities import (
    Capability,
    CapabilityError,
    OutcomeKind,
    Resolver,
...
class CapabilityResolverTests(unittest.TestCase):
    ...
    def test_declined_formula_install_is_terminal(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            resolver = Resolver(
                ...
                output=io.StringIO(),
                which=lambda executable, _path: (
                    "/fake/brew" if executable == "brew" else None
                ),
            )
    ...
    def test_native_tool_build_declares_xmake_and_sdl_requirements(self) -> None:
        self.assertEqual(
            requirements_for(["check", "tools"]),
            (Capability.XMAKE, Capability.PKG_CONFIG, Capability.SDL3),
        )
    ...
    def test_setup_includes_shellcheck(self) -> None:
        self.assertIn(Capability.SHELLCHECK, requirements_for(["setup"]))
    ...
    def test_ci_notary_secrets_need_no_keychain_repair(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            resolver = Resolver(
                ...
                run=lambda command, **_kwargs: self.fail(
    ...
    def test_profile_install_discovers_downloads(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            script = Path(__file__).resolve().parents[2] / "Scripts/ojd"
            result = subprocess.run(
                [str(script), "signing", "install-profiles"],
                env={
                    "HOME": str(home),
