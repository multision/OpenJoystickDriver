...
func runDS3InputChecks() throws {
  ...
  let axisEvents = try axisParser.parse(
    data: makeDS3Report(leftX: 255, leftY: 0, rightX: 0, rightY: 255, l2Analog: 255, r2Analog: 128)
  )
...
func runSwitchProTransportAndMappingCheck() throws {
  ...
  require(
    bluetoothStartup.map(\.reportID) == [0x01, 0x01, 0x01, 0x01, 0x01],
    "Switch Pro Bluetooth should send subcommand startup reports"
  )
  require(
    bluetoothStartup.map { $0.bytes[10] } == [0x03, 0x40, 0x48, 0x10, 0x10],
  ...
  require(
    startupReports.map(\.reportID) == [0x80, 0x80, 0x80, 0x80, 0x01, 0x01, 0x01, 0x01, 0x01],
