...
struct MotionOptionsTests {
  @Test
  func gyroConsumptionCanBeDisabledAndSurvivesPartialUpdates() throws {
    ...
    let unchanged = try MappingProfileEditor.gyroOutput(MappingOptions([]), defaultValue: output)
    #expect(unchanged == output)
    let invalid = try MappingOptions(["--gyro-consume-activation", "yes"])
    #expect(throws: MappingCommandError.self) { try MappingProfileEditor.gyroOutput(invalid) }
  }

  @Test
  func partialUpdatePreservesOtherSettingsAndCanDisableBooleans() throws {
    let original = RemappingMotionTuning(
      space: .world,
      pitchSensitivity: 3,
      invertYaw: true,
      smoothingHalfTimeMs: 20
    )
    let options = try MappingOptions([
      "--motion-yaw-sensitivity", "4", "--motion-invert-yaw", "false", "--motion-automatic-bias",
      "false",
    ])
  ...
  @Test
  func leanAndSteeringCanBeEnabledUpdatedAndRemovedIndependently() throws {
    let configured = try MappingProfileEditor.motionTuning(
      MappingOptions([
  ...
  @Test(arguments: [
    ["--motion-space", "unknown"], ["--motion-invert-pitch", "yes"],
    ["--motion-yaw-sensitivity", "nan"], ["--motion-smoothing-half-time-ms", "-1"],
  ...
  func invalidOptionsAreRejected(arguments: [String]) throws {
    ...
    #expect(throws: (any Error).self) { try MappingProfileEditor.motionTuning(options) }
  }
}
