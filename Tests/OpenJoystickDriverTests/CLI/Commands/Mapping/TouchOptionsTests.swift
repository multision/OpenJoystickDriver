...
struct TouchOptionsTests {
  @Test
  func partialUpdatePreservesOtherSurfaceAndProfileSettings() throws {
    ...
    let updated = try MappingProfileEditor.updating(
      original,
      options: MappingOptions(["--touch-surface", "left", "--touch-pointer-sensitivity", "750"])
    )
    ...
    let removed = try MappingProfileEditor.updating(
      updated,
      options: MappingOptions(["--touch-surface", "left", "--touch-mode", "none"])
    )
    #expect(removed.touchMappings == [right])
  }

  @Test
  func createsTypedMappingWithDefaults() throws {
    let mappings = try MappingProfileEditor.touchMappings(
      MappingOptions(["--touch-surface", "primary", "--touch-mode", "pointer"])
    )
  ...
  @Test(arguments: [
    ["--touch-mode", "pointer"], ["--touch-surface", "unknown"],
