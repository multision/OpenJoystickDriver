...
struct PassiveUSBDescriptorBoundaryTests {
  @Test
  func duplicateEndpointInsideValidAlternateIsRejected() {
    let bytes: [UInt8] = [
  ...
  @Test
  func duplicateSSPCompanionIsRejected() {
    let bytes: [UInt8] = [
  ...
  @Test
  func SSPCompanionAfterNonIsochronousEndpointIsRejected() {
    let bytes: [UInt8] = [
  ...
  @Test
  func trailingOneByteDescriptorFragmentIsRejected() {
  ...
  @Test
  func descriptorOwnershipAndSSPMalformedBoundariesAreRejected() throws {
